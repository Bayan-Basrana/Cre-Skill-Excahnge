...
public class TalentController {
    ...
    @PostMapping("/add2")
    public ResponseEntity add2 (@RequestBody @Valid List<Talent> talents , Errors errors){
        if (errors.hasErrors()){
    ...
    @PostMapping("/add")
    public ResponseEntity add (@RequestBody @Valid Talent talent , Errors errors){
        if (errors.hasErrors()){
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
    ...
    public ResponseEntity update (@PathVariable Integer userid ,@RequestBody @Valid Talent talent , Errors errors ){
        if (errors.hasErrors()){
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
