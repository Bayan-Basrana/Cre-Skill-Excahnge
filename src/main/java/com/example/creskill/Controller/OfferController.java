...
public class OfferController {
    ...
    @PostMapping("/add")
    public ResponseEntity add (@RequestBody @Valid Offer offer , Errors errors){
        if (errors.hasErrors()){
    ...
    public ResponseEntity update (@PathVariable Integer offerId ,@RequestBody @Valid Offer offer , Errors errors ){
        if (errors.hasErrors()){
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
