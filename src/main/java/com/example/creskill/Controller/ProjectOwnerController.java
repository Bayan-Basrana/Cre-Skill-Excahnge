...
public class ProjectOwnerController {
    ...
    @PostMapping("/add2")
    public ResponseEntity add2 (@RequestBody @Valid List<ProjectOwner> owners , Errors errors){
        if (errors.hasErrors()){
    ...
    @PostMapping("/add")
    public ResponseEntity add (@RequestBody @Valid ProjectOwner projectOwner , Errors errors){
        if (errors.hasErrors()){
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
    ...
    public ResponseEntity update (@PathVariable Integer ownerId ,@RequestBody @Valid ProjectOwner projectOwner , Errors errors ){
        if (errors.hasErrors()){
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
...
@PutMapping("/checkAndVerifyOwner/{ownerId}")
public ResponseEntity checkAndVerifyOwner (@PathVariable Integer ownerId){
projectOwnerService.checkAndVerifyOwner(ownerId);
