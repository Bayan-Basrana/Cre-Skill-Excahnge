...
//حساب عدد المشاريع المكتملة لصاحب مشروع
    @Query("select count (p.projectId )from Projects p where p.status='completed' and p.userid=?1")
    Integer countCompletedProjectForOwner (Integer userid);
